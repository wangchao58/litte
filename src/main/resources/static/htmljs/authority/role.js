...
jQuery(function($) {
    ...
    jQuery(grid_selector).jqGrid({
        ...
        mtype: "post",
        colModel:[
            {label:'roleId',name:'roleId', editable:false,hidden:true },
            {label:'角色名',name:'roleName', editable: true },
    ...
    function operation(cellvalue, options, rowObject) {
        ...
        htmlstr +="<div class='visible-md visible-lg hidden-sm hidden-xs btn-group'>"
            ...
            + rowObject.roleId
            + "')><i class='icon-cog bigger-120'></i>授权</button></div>";
        return htmlstr;
    }
...
function deletes() {
    ...
    $(ids).each(function (index, id){
        //由id获得对应数据行
        var name= jQuery('#grid-table').jqGrid('getCell',id,'roleId');//name是colModel中的一属性
        row += name+",";
    });
...
function openEditDiag(uuid) {
    ...
    var indext = layer.open({
        ...
        fixed: false, //不固定
        maxmin: true,
        content: '/role/updRoleView?id='+uuid,
        btn:['确定','取消'],
...
function addRole(){
    $.ajax({
        url: '/role/accreditAdd',
        data: $("#roleAddMenu").serialize(),
        type: "POST",
        async: true,
        dataType: "json",
        success: function (result) {
