...
function openEditDiag(uuid) {
    ...
    var indext = layer.open({
        ...
        fixed: false, //不固定
        maxmin: true,
        content: '/territory/updTerritoryView?id='+uuid,
        btn:['确定','取消'],
...
function openAddDiag() {
    ...
    var indext = layer.open({
        ...
        fixed: false, //不固定
        maxmin: true,
        content: '/territory/addTerritoryView',
        btn:['确定','取消'],
        yes: function (layero, index) {
            var iframeWin = window[index.find('iframe')[0]['name']]; //得到iframe页的窗口对象，执行iframe页的方法：iframeWin.method();
            //调用授权提交方法
...
function mySubmit(){
    ...
    $.ajax({
        url: '/territory/addTerritory',
        data:$("#updateTerritory").serialize(),
        type: "POST",
        async: true,
        dataType: "json",
        success: function (result) {
