...
jQuery(function($) {
    ...
    jQuery(grid_selector).jqGrid({
        ...
        colModel:[
            ...
            {label:'职位',name:'job', editable: true },
            {label:'地址',name:'sear', editable: true },
            {label:'薪资',name:'salary',  editable: true},
            {label:'时间',name:'createTime', editable: true, formatter:formatDatebox},
...
function openEditDiag(uuid) {
    ...
    var indext = layer.open({
        ...
        fixed: false, //不固定
        maxmin: true,
        content: '/recruit/updRecruitView?id='+uuid,
        btn:['确定','取消'],
...
function openAddDiag() {
    ...
    var indext = layer.open({
        ...
        fixed: false, //不固定
        maxmin: true,
        content: '/recruit/addRecruitView',
        btn:['确定','取消'],
        yes: function (layero, index) {
            var iframeWin = window[index.find('iframe')[0]['name']]; //得到iframe页的窗口对象，执行iframe页的方法：iframeWin.method();
            //调用授权提交方法
